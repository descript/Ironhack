  ...
  def start_game
    ...
    @player.set_location(@rooms[:jailcell]) # : @player.set_location(@rooms[@load_game[0].chomp.to_sym]) #puts player into the  jailcell as starting room
      while @char != "Q"
        ...
        when "INVENTORY?"
          puts "You have #{@player.check_inventory.join(', ')} in your inventory"
        when "EXITS"
        ...
        when "LOAD GAME"
          load_game
        when "LOOK AROUND"
          @player.find_items.empty? ? (puts "The room is empty") : (puts "You see #{@player.find_items.join(', ')} in the room")
        when "GET ITEM"
          @player.find_items.empty? ? (puts "Thare aint nuthin ta git!") : @player.pick_up_item
        when "DROP ITEM"
...
class Player
  ...
  def find_items #returns an array
    @location[:objects] ? @location[:objects].each {|x| x} : false
  end
  ...
  def put_down_item(thing)
    item_to_drop = @inventory.select {|object| object==thing}.join('') #assigns item to drop to be 'thing' in the inventory array
...
rooms = {
  ...
  :yard => {:title=>"yard", :exits => {"N"=>'main_hallway',"E" => 'small_yard',"S"=>'garage',"W"=>'office'},:objects=>[],:hint=>"This courtyard is open, you can go in any diretion",:desc=>"You step outside into a sunny, grassy courtyard. All sides are either fenced in or bordering buildings."},
